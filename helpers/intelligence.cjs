...
// Phase 4 (ADR-054): prefer project-local .claude-flow; fall back to global
// so intelligence runs correctly for root/home sessions too.
const _localCF = path.join(process.cwd(), '.claude-flow');
const DATA_DIR = fs.existsSync(_localCF)
  ? path.join(_localCF, 'data')
  : path.join(require('os').homedir(), '.claude-flow', 'data');
